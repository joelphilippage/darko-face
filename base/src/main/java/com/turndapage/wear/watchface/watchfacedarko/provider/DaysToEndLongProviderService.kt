...
class DaysToEndLongProviderService : ComplicationProviderService() {
    ...
    override fun onComplicationUpdate(complicationId: Int, type: Int, manager: ComplicationManager?) {
        ...
        if(ComplicationData.TYPE_LONG_TEXT == type) {
            val complicationData: ComplicationData = ComplicationData.Builder(ComplicationData.TYPE_LONG_TEXT)
                    .setLongText(ComplicationText.plainText("$days:$hours:$minutes:$seconds"))
                    .setLongTitle(ComplicationText.plainText("That is when the world will end"))
                    .build()
